...
let db = new sqlite3.Database('ta_hours_queue.db', sqlite3.OPEN_READWRITE, (err) => {
    if (err) {
        ...
        db.run('CREATE TABLE sessions(session_id INTEGER PRIMARY KEY, public_key varchar(255), private_key varchar(255), active int, session_name varchar(255), room varchar(255), tas varchar(255), starttime int, endtime int)', 
        (err) => {
            if (err) {
                // Table already created
            }else{  
                ...
                for (let t = 0; t < 5000; t++){
                    ...
                    while(created === false){
                        ...
                        if (!((publicKey in publicKeys) & (privateKey in privateKeys))){
                            let insert = 'INSERT INTO sessions(public_key, private_key, active, session_name, room, tas, starttime, endtime) VALUES (?,?,?,?,?,?,?,?)';
                            db.run(insert, [publicKey, privateKey, 0, "None", "None", "None", 0, 0]);
